...
def upload_pic(current_user):
    ...
    if file.filename == '' or file.filename is None:
        logger.error(f"""User \"{current_user['username']}\" upload failed:
    ...
    # File Validation (Backend)
        ...
        return jsonify({'error': """Invalid file type.
                        Only .JPEG, .JPG, .PNG files are allowed."""}), 400
...
def change_password(current_user):
    ...
    if not is_password_complex(new_password):
        ...
        return jsonify({'error': """Password must be at least 8 characters long and
                        include 1 uppercase letter,
                        1 number, and 1 special character."""}), 400
